...
var beerTime = new Date();
beerTime.setHours(16);
...
// Get setting.
app.get("/appset/:setting", function(req, res)
{
  var settingName = req.params.setting;
  if (settings[settingName])
  {
    res.json({ value: settings[settingName]});
  }
  else {
    res.send(404);
  }
...
// Save setting.
app.post("/appset", function(req, res)
{
  var settingName = req.body.setting;
  if (settings[settingName])
  {
    settings[settingName] = req.body.value;
    topic.pub("evt/push", {
      type: "beertime",
      beerTime: req.body.value
    });
  }
  else {
    res.send(404, "App setting " + settingName + " not found!");
  }
});
